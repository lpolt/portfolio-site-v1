...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function chooseSrc(entries) {
    ...
    // fallback to largest
    return entries.items.length ? entries.items[entries.items.length - 1].url : null;
  }
  ...
  const onIntersect = (entries, obs) => {
    entries.forEach(entry => {
      ...
      if (bgset) {
        ...
        const elementWidth = el.clientWidth || parseInt(getComputedStyle(el).width, 10) || window.innerWidth;
  ...
  document.querySelectorAll('[data-bg],[data-bgset]').forEach(el => observer.observe(el));
});
