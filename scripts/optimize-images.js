...
async function genDrem() {
  ...
  if (!fs.existsSync(src)) {
    console.warn('drem-crop.jpg not found, skipping drem tasks');
    return;
  }
...
async function genPfp() {
  ...
  if (!fs.existsSync(src)) {
    console.warn(srcName, 'not found, skipping pfp tasks');
    return;
  }
...
async function genLogos() {
  const logos = [
    'Drem-Labs-Wordmark.png',
    'bscu-logo.png',
    'bscu-logo-w.png',
    'flourish-wordmark.png',
    'flourish-wordmark-w.png',
    'laptop-96.png',
  ];
